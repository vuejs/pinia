...
### Using classes for state objects

If you use classes to define the state object for your store, take care to explicitly initialize all properties or you may have trouble with reactivity and the `mapState()` helper method.
...
const useMyStore = defineStore({
  state(): MyState { return new MyState() }
  //...
}
...
//...
  computed: {
    ...mapState(useMyStore, {
      myValue: state => state.someValue; 
    })
  }
//...
```

When the MyState object is created the `someValue` property key will not be present as it does not have an explicit value. When `mapState()` is then called, the `myValue` computed property will exist but will not be reactive as it could not find a property key to map to. Adding the `someValue` property to the state object at a later time via `Vue.set()` will not remedy this problem.

The solution is to ensure that all properties have an initial value, even those that should be `undefined`.
